...
do
	...
	humann -i "${prefix}_FASTP_R1_001.fastq.gz" --threads 38 --remove-temp-output --taxonomic-profile JOHI_merged_abundance_table.txt --output-basename "${prefix}" -o OUTPUT_JOHI_SUBSET --o-log "${prefix}".log -v
done
